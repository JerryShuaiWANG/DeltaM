...
def GetTextLinesWords(TextFilePath, Delimiter, QuoteChar, IgnoreHeaderLine):
    ...
    if not os.path.exists(TextFilePath):
        PrintError("The text file file, %s, doesn't exist.\n" % (TextFilePath))
...
def ExpandFileNames(FilesSpec, Delimiter = ","):
    ...
    """
    FileNames = []
    if not len(FilesSpec):
        return FileNames

    for FileSpec in FilesSpec.split(Delimiter):
        FileSpec = FileSpec.strip()
        if re.search("(\*|\?)", FileSpec, re.I):
            FileNames.extend(glob.glob(FileSpec))
        else:
            FileNames.append(FileSpec)
...
def GetWallClockAndProcessorTime():
    ...
    """
    return (time.time(), _GetProcessorTime())

def _GetProcessorTime():
    """Get processor time """

    if sys.version_info[0] >= 3 and sys.version_info[1] >= 3:
        ProcessorTime = time.process_time()
    else:
        ProcessorTime = time.clock()
...
def GetFormattedElapsedTime(StartingWallClockTime, StartingProcessorTime):
    ...
    ElapsedProcessorTime = _GetProcessorTime() - StartingProcessorTime
    
    ElapsedTime = "%d wallclock secs ( %.2f process secs)" % (ElapsedWallClockTime, ElapsedProcessorTime)
    
    return ElapsedTime

def GetFormattedFileSize(FileName, Precision = 1):
    """Get file size  as a string in the following format: %.*f <bytes, KB, MB,
    GB>
    ...
    if Size < 1024:
        ...
        SizeDenominator = 1024*1024*1024
        SizeSuffix = "GB"
    else:
        SizeDenominator = 1
        SizeSuffix = "bytes"
    ...
    FormattedSize = "%.*f %s" % (Precision, Size, SizeSuffix)
...
def ObjectToBase64EncodedString(Object):
    ...
    return None if Object is None else base64.b64encode(pickle.dumps(Object)).decode()
...
def ObjectFromBase64EncodedString(EncodedObject):
    ...
    return None if EncodedObject is None else pickle.loads(base64.b64decode(EncodedObject))
...
def ProcessOptionMultiprocessingParameters(ParamsOptionName, ParamsOptionValue):
    ...
    ParamsInfo = {'ChunkSize': 'auto', 'InputDataMode' : 'Lazy', 'NumProcesses': 'auto'}
    
    if re.match("^auto$", ParamsOptionValue, re.I):
        # No specific parameters to process except for parameters with possible auto value...
        _ProcessOptionMultiprocessingParameters(ParamsInfo, ParamsOptionName, ParamsOptionValue)
        return ParamsInfo
    ...
    for Index in range(0, len(ParamsOptionValueWords), 2):
        ...
        if re.match("^InputDataMode$", ParamName, re.I):
            if re.match("^Lazy$", Value, re.I):
                ParamValue = "Lazy"
            elif re.match("^InMemory$", Value, re.I):
                ParamValue = "InMemory"
            else:
                PrintError("The parameter value, %s, specified for parameter name, %s, using \"%s\" option is not a valid value. Supported values: Lazy or InMemory" % (Value, Name, ParamsOptionName))
        elif re.match("^NumProcesses$", ParamName, re.I):
            if not re.match("^Auto$", Value, re.I):
                ...
                if Value > mp.cpu_count():
                    PrintWarning("The parameter value, %s, specified for parameter name, %s, using \"%s\" option is greater than number of CPUs, %s, returned by mp.cpu_count()." % (Value, Name, ParamsOptionName, mp.cpu_count()))
        else:
            if not re.match("^Auto$", Value, re.I):
                Value = int(Value)
                if Value <= 0:
...
def _ProcessOptionMultiprocessingParameters(ParamsInfo, ParamsOptionName, ParamsOptionValue):
    """Process parameters with possible auto values."""

    # NumProcesses parameter...
    Value = ParamsInfo["NumProcesses"]
    ParamsInfo["NumProcesses"] = mp.cpu_count() if re.match("^auto$", Value, re.I) else int(Value)

    # ChunkSize parameter...
    Value = ParamsInfo["ChunkSize"]
    if re.match("^auto$", Value, re.I):
        Value = None if  re.match("^InMemory$", ParamsInfo["InputDataMode"], re.I) else 1
    else:
        Value = int(Value)
    ParamsInfo["ChunkSize"] = Value
